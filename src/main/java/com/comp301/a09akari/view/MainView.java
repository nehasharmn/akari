...
public class MainView implements FXComponent, ModelObserver {
  ...
  public Parent render() {
    ...
    if (shouldShowHeader()) {
      ...
      header.setStyle(
          "-fx-font-size: 40px; -fx-text-fill: orange; -fx-font-weight: bold; -fx-padding: 10;");
  ...
  private boolean shouldShowHeader() {
    int gridHeight = model.getActivePuzzle().getHeight();
    int gridWidth = model.getActivePuzzle().getWidth();
