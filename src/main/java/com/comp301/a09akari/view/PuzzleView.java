...
public class PuzzleView implements FXComponent {
    ...
    public Parent render() {
        ...
        for (int i = 0; i < model.getThePuzzle().getHeight(); i++) {
            for (int j = 0; j < model.getThePuzzle().getWidth(); j++) {
                switch (model.getThePuzzle().getCellType(i, j)) {
                    case CLUE:
    ...
    private void clueCFormat(GridPane board, int i, int j, int size) {
        ...
        Text text = new Text("" + model.getThePuzzle().getClue(i, j));
